...
public class SeamCarver {
     ...
     *                                   or equal to the image width, if <em>y</em>
     *                                   is greater than or
     *                                   equal to the image height, or if <em>x</em>
     *                                   or <em>y</em> are
     *                                   negative.
     */
     ...
     *                                  match the picture width, if an index in the
     *                                  given <em>seam</em>
     *                                  is negative or is taller than the picture,
     *                                  or if two adjacent
     *                                  entries in the given <em>seam</em> differ by
     *                                  more than 1.
     */
     ...
     * @throws NullPointerException     if the given <em>seam</em> is {@code null}.
     * @throws IllegalArgumentException if the given <em>seam</em> does not
     *                                  match the picture height, if an index in the
     *                                  given <em>seam</em>
     *                                  is negative or is wider than the picture, or
     *                                  if two adjacent
     *                                  entries in the given <em>seam</em> differ by
     *                                  more than 1.
     */
    ...
    public static void main(String[] args) {
        final String INSTRUCTION = "Usage: java SeamCarver <imagefile> <h|w> <numberOfSeamsToRemove>" +
                "\te.g. java SeamCarver example.jpg h 200";
        if (args.length == 1) {
            if ("-h".equals(args[0])) {
                System.out.println(INSTRUCTION);
                return;
            } else {
                System.out.println("Unrecognized Option. Use \'-h\' for help. ");
            }
        } else if (args.length != 3) {
            System.out.println("Invalid params. Use \'-h\' for help. ");
            return;
        }
        String filePath = args[0];
        boolean horizontal = args[1].charAt(0) == 'h';
        int iterations = Integer.parseInt(args[2]);

        Picture picture = new Picture(filePath);
        SeamCarver seamCarver = new SeamCarver(picture);
        if (horizontal)
            seamCarver.autoRemoveHorizontalSeams(iterations);
        else
            seamCarver.autoRemoveVerticalSeams(iterations);
